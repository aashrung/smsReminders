...
const createUser = async (req, res) => {
    try {
        let { firstName, lastName, reminderName, reminderTask, messages, smsSentToday, mobileNumber } = req.body
        ...
        if (!reminderName || typeof reminderName != string) return res.status(400).json({ message: "Reminder Name is required!" })
        ...
        if (!smsSentToday || typeof smsSentToday != boolean) return res.status(400).json({ message: "smsSentToday is required!" })
